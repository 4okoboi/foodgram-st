...
from .serializers import RecipeListSerializer, RecipeCreateSerializer, RecipeInUserSerializer, IngredientSerializer


class RecipeAuthorFilter(FilterSet):
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    filterset_class = RecipeAuthorFilter
    ...
    def favorite(self, request, pk=None):
        ...
        if request.method == 'POST':
            ...
            serializer = RecipeInUserSerializer(recipe, context=self.get_serializer_context())
...
def download_shopping_cart(request):
    ...
    for cart_item in cart_items:
        for ing in cart_item.recipe.recipe_ingredients.all():
            key = (ing.ingredient.name, ing.ingredient.measurement_unit)
            ingredients[key] += ing.amount

    ing_w_measure_units = [f"{name} — {amount} {unit}." for (name, unit), amount in ingredients.items()]
    content = "\n".join(ing_w_measure_units)
